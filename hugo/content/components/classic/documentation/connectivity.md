---
title: Connectivity
layout: classic-doc
---

Apache ActiveMQ Classic supports a large number of [Cross Language Clients](cross-language-clients), a wide range of [Protocols](protocols), and many [Containers](containers).

### Protocols

- [AMQP](amqp)
- [AUTO](auto)
- [MQTT](mqtt)
- [OpenWire](openwire)
- [REST](rest)
- [Stomp](stomp)
- [WebSockets](websockets)
- [XMPP](xmpp)

### Cross Language Clients

- [C Integration](c-integration)
- [Delphi and FreePascal](delphi-and-freepascal)
- [ActiveMQ Classic C++ Clients](activemq-c-clients)

### Containers

- [Spring Support](spring-support)
- [OSGi Integration](osgi-integration)
- [Tomcat](tomcat)
- [JBoss Integration](jboss-integration)
- [Geronimo](geronimo)
- [J2EE](j2ee)
- [Resource Adapter](resource-adapter)
- [JNDI Support](jndi-support)

### Bridging

- [JMS to JMS Bridge](jms-to-jms-bridge)
- [Ajax](ajax)
